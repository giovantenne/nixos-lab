{ pkgs, lib, labSettings, labConfig, ... }:

let
  renderDesktopFavorites = favorites:
    "[${lib.concatMapStringsSep ", " (app: "'${app}'") favorites}]";
  studentFavoriteApps = renderDesktopFavorites labSettings.software.desktop.studentFavorites;
  staffFavoriteApps = renderDesktopFavorites labSettings.software.desktop.staffFavorites;
  veyonFavoritesScript =
    if labConfig.features.veyon.enable then
      ''
...
in
{
  ...
  services.desktopManager.gnome.extraGSettingsOverrides = ''
    ...
    favorite-apps=${staffFavoriteApps}
  ...
  environment.etc."lab/gnome-user-setup.sh" = {
    text = ''
      ...
      case "''${USER:-}" in
        ${labSettings.studentUser}|${labSettings.adminUser}|${labSettings.teacherUser}) ;;
        *) exit 0 ;;
      esac
